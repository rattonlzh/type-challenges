...
type Except<S, K extends string> = S extends `${infer A}${K}${infer B}` ? `${A}${B}` : S
type AllCombinations<S, U = Split<S>> = U extends string ? U extends '' ? U : U | `${U}${AllCombinations<Except<S, U>>}` : never
